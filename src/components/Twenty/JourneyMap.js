...
const defaultMoments = [
  {
    time: "4:55 PM",
    place: "Coworking",
    title: "Opens Twenty, cross-checks with Slack.",
    anxiety: 5,
    apps: ["Twenty", "Slack"],
    fires: 3,
    type: "stale",
  ...
  },
  {
    ...
    friction:
      "Maya never appeared under Gopuff because she is not part of the explicit graph. Her Person record was auto-created from email and has no direct link to the Gopuff Opportunity. Company and Opportunity views resolve through explicit links, so she is excluded even though her email domain and context clearly tie her to Gopuff.",
    fix: "There should be a way to surface records related to the focus through indirect paths, not just direct links.",
  },
  {
    time: "5:11 PM",
    place: "Coworking",
    title: "Unblocks the contract in last two minutes",
    anxiety: 4,
  ...
  },
  {
    time: "5:17 PM",
    place: "Coworking",
    title: "Leaves with the account saved and works to keep Twenty up-to-date",
    anxiety: 3,
    apps: ["Gmail", "Notes", "Twenty"],
    fires: 3,
    type: "lost",
    what: "Sent folder is clean. Marcus checks a quick Notes checklist he keeps for end-of-day sanity. Everything is done. Twenty still shows open Tasks. He closes the Tasks manually, because that's what he has to do to keep Twenty up-to-date.",
    friction:
...
const buildFillGradient = (chart, topVar, bottomVar) => {
  ...
  const topColor = readVar(topVar);
  const bottomColor = readVar(bottomVar);

  // CSS vars may resolve to "" if the theme hasn't applied yet, or if the
  // var is defined on a class scope rather than :root. addColorStop throws
  // on an empty/invalid color string, so bail out — Chart.js accepts null.
 ...
 */
export const JourneyMap = ({ customMoments }) => {
  ...
  // Inject the keyframes once. Avoids needing a plugin or global stylesheet.
  useEffect(() => {
    if (typeof document === "undefined") return;
    const id = "journey-map-keyframes";
    if (document.getElementById(id)) return;
    const style = document.createElement("style");
    style.id = id;
    style.textContent = `
      @keyframes journey-fade-up {
      ...
      .journey-scroll-x::-webkit-scrollbar {
        display: none;
        width: 0;
        height: 0;
      }
  ...
  const fadeStyle = { animation: "journey-fade-up 280ms ease-out" };
  ...
  // Skip the initial run so mounting doesn't yank the page down.
  const dotScrollMountedRef = useRef(false);
  useEffect(() => {
    if (!dotScrollMountedRef.current) {
      dotScrollMountedRef.current = true;
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div
        ref={dotRowRef}
        className={isMobile ? "journey-scroll-x" : ""}
        style={{
          display: "grid",
